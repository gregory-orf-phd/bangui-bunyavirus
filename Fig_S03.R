...
N23_ML_L_codon <-
  read.iqtree(
    file = "data/rooted.aligned.serogroups_of_interest_L_AA.fasta.treefile"
  ) %>%
...
N23_ML_M_codon <-
  read.iqtree(
    file = "data/rooted.aligned.serogroups_of_interest_M_AA.fasta.treefile"
  ) %>%
    groupOTU(., c("Bangui|1970", "Bangui|2017", "Nyangole|2013")) %>%
    ggtree(
      .,
...
N23_ML_S_codon <-
  read.iqtree(
    file = "data/rooted.aligned.serogroups_of_interest_S_AA.fasta.treefile"
  ) %>%
    groupOTU(., c("Bangui|1970", "Bangui|2017", "Nyangole|2013")) %>%
    ggtree(
      .,
...
Panel_A <-
  ...
  labs(caption = "L Polyprotein, AA Alignment")
  # guides(
...
Panel_B <-
  ...
  labs(caption = "M Polyprotein, AA Alignment")
  # guides(
  #   fill = guide_legend(ncol = 2, title.position = "top", title.hjust = 0.5, title = "Serogroup")
  # )
...
Panel_C <-
  ...
  labs(caption = "S Protein, AA Alignment")
  # guides(
  #  fill = guide_legend(ncol = 2, title.position = "top", title.hjust = 0.5, title = "Serogroup")
  # ) +
